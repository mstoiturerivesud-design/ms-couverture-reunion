// ================================================================
// AMÉLIORATIONS — MS Couverture Réunion
// ================================================================
// INSTALLATION : Ajoutez  <script src="ameliorations.js"></script>
// juste avant </body> dans index.html, puis committez ce fichier.
//
// CONFIGURATION : modifiez WA_NUMBER avec votre numéro WhatsApp.
// ================================================================
...
(function () {
  ...
  // ======= CONFIG =======
  var WA_NUMBER = '262XXXXXXXXX'; // Votre numéro sans le + (ex: 262693123456)
  var WA_MSG    = encodeURIComponent('Bonjour, je souhaite un devis pour ma toiture à La Réunion.');
  var OG_IMAGE  = 'https://couvreur974.fr/og-image.jpg';
  // ======================
  ...
  waCSS.textContent = [
   ...
   '.wa-float-btn{width:56px;height:56px;border-radius:50%;background:#25d366;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 20px rgba(37,211,102,.45);transition:transform .2s,box-shadow .2s;}',
    '.wa-float:hover .wa-float-btn{transform:scale(1.08);}',
  ...
  avisCSS.textContent = [
    ...
    '.avis-stars{color:#f59e0b;font-size:20px;margin-bottom:14px;}',
  ...
  var avis = [
    ...
    {nom:'Joëlle M.',ville:'Saint-Leu',texte:'Très satisfaite. Professionnel, propre, finitions irréprochables. Merci !'}
  ];

  var grid = avis.map(function(a){
    return '<article class="avis-card"><div class="avis-stars">★★★★★</div>' +
      '<p class="avis-text">« '+a.texte+' »</p>' +
      '<div class="avis-name">'+a.nom+'</div>' +
      '<div class="avis-ville">'+a.ville+'</div></article>';
  }).join('');

  var avisSection = document.createElement('section');
  avisSection.className = 'section avis-section';
  avisSection.innerHTML = '<div class="container">' +
    '<div class="section-kicker">Avis clients</div>' +
    '<h2 class="section-title">Ce que disent nos clients.</h2>' +
    '<div class="avis-grid">'+grid+'</div></div>';
  ...
  faqEl.textContent = JSON.stringify({
    ...
    mainEntity:[
      {'@type':'Question',name:'Combien coûte une peinture de toiture à La Réunion ?',acceptedAnswer:{'@type':'Answer',text:"Le coût dépend de la superficie et de l'état du support. Contactez-nous pour un devis gratuit."}},
      {'@type':'Question',name:'Intervenez-vous à Saint-Gilles-les-Bains et Saint-Paul ?',acceptedAnswer:{'@type':'Answer',text:"Oui, nous couvrons l'Ouest réunionnais : Saint-Gilles, Saint-Paul, La Saline, Saint-Leu, Le Port et La Possession."}},
      {'@type':'Question',name:'Quels types de toitures prenez-vous en charge ?',acceptedAnswer:{'@type':'Answer',text:'Spécialisés en toiture tôle : peinture, étanchéité, nettoyage, démoussage, resserrage des fixations.'}},
      {'@type':'Question',name:'Comment obtenir un devis ?',acceptedAnswer:{'@type':'Answer',text:'Envoyez votre commune et quelques photos via le formulaire. Nous planifions une visite rapidement.'}}
    ]
  });
  ...
  if (nav) {
    ...
    navCSS.textContent = 'nav{transition:background .3s,box-shadow .3s;}nav.nav--scrolled{background:rgba(255,248,236,.95)!important;backdrop-filter:blur(12px);box-shadow:0 2px 16px rgba(0,0,0,.08);}';
